            ...
            <ul className='flex gap-4'>
              {paymentsData.map((listItem) => (
                <li key={listItem.title}>
                  <Link className='relative text-lg' to={pathOr("#", ["href"], listItem)}>
                    <img src={listItem.Icon} alt='payement logo' height={12} width={24} className='object-contain' />
                  </Link>
                </li>
              ))}
            </ul>
            ...
            <ul className='flex gap-4'>
              {socialLinks.map((listItem) => (
                <li key={listItem.href}>
                  <Link className='text-lg' to={pathOr("#", ["href"], listItem)}>
