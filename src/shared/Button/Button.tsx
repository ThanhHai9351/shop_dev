...
const Button: FC<ButtonProps> = ({
  ...
  if (href) {
    ...
    return (
      <NavLink to={href} className={CLASSES} onClick={onClick}>
        {showPointer ? (
          <>
          ...
          <span className='inline-flex items-center justify-center gap-2'>{children || `This is a Link`}</span>
        )}
      </NavLink>
    )
  }
  ...
  return (
    ...
    >
      ...
      {showPointer ? (
        ...
        <span className='inline-flex items-center justify-center gap-2'>{children || `This is Button`}</span>
      )}
    </button>
  )
}
