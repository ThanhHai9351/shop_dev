    ...
    >
      <div className='absolute left-0 top-0 -z-10 size-full'>
        <img
          src={image}
          alt='bg-graphic'
          className='scale-105 object-cover object-center transition-transform duration-700 group-hover:scale-100 dark:opacity-60 h-full'
        />
      </div>
      {isFirst ? (
        <div>
          <span className='text-xs text-primary dark:text-white'>
            <span className='inline-block size-1 rounded-full bg-green-600 dark:bg-white' /> IN STOCK NOW
          </span>
          <h2 className='mb-2 text-4xl font-bold leading-tight md:mt-16 md:w-[70%] lg:mt-10'>{title}</h2>
        ...
        <div className='flex  h-full flex-col'>
          ...
          <h3 className='w-2/3 font-bold leading-tight tracking-tight'>{title}</h3>
        </div>
      )}
    </Link>
  )
