const ProductCharacteristicsTab = ({ attrs }: { attrs: Object }) => {
  return (
    <div className=''>
          ...
          <ul className='divide-y divide-neutral-300  dark:divide-neutral-400'>
            {attrs ? (
              Object.entries(attrs).map(([key, value]) => (
                <li className='mb-4 pt-4' key={key}>
                  <div className='flex justify-between gap-3'>
                    <div className='basis-1/5'>
                      <span className='font-medium'>{key}</span>
                    </div>
                    <div className='basis-4/5'>
                      <p className='text-neutral-500 dark:text-neutral-300'>{String(value)}</p>
                    </div>
                  </div>
                </li>
              ))
            ) : (
              <p className='text-neutral-500 dark:text-neutral-300'>Updating...</p>
            )}
            {/* {productCharacterData.map((listItem) => (
              <li className='mb-4 pt-4 ' key={pathOr("", ["title"], listItem)}>
                <div className='flex justify-between gap-3'>
                  <div className='basis-1/5'>
                    <span className='font-medium'>{pathOr("", ["title"], listItem)}</span>
                  </div>
                  <div className='basis-4/5'>
                    <p className='text-neutral-500  dark:text-neutral-300'>{pathOr("", ["text"], listItem)}</p>
                  </div>
                </div>
              </li>
            ))} */}
