...
const CountriesMobile = () => {
  ...
  const activeCountry = countriesData.find((country) => country.code === activeCountryCode)
  return (
    <div className='px-3 font-medium'>
      <Menu as='div' className='relative inline-block md:pt-2'>
        <Menu.Button className='inline-flex items-center gap-2 text-sm'>
          {activeCountry && (
            <span className='relative inline-block size-4 overflow-hidden rounded-full'>
              <img src={activeCountry.flagUrl} alt='country flag' className='object-cover' />
            </span>
          )}
        ...
        >
          <Menu.Items className='divide-gray-100 absolute bottom-9 left-0 mt-2 h-80 w-52 origin-top-right divide-y overflow-scroll rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none dark:bg-neutral-900'>
                  ...
                  <div className='flex justify-between'>
                    <span className='inline-flex items-center gap-2'>
                      <span className='relative inline-block size-6 overflow-hidden rounded-full'>
                        <img src={item.flagUrl} alt='country flag' className='object-cover' />
                      </span>
