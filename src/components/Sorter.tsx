...
const Sorter = () => {
  const languages = [
    { key: 1, value: "Alphabetically A-Z", label: "Alphabetically A-Z" },
    { key: 2, value: "Alphabetically Z-A", label: "Alphabetically Z-A" },
    { key: 3, value: "Increase Price", label: "Increase Price" },
    { key: 4, value: "Decrease Price", label: "Decrease Price" },
  ]
      ...
      <Popover as='div' className='relative inline-block w-full'>
        <Popover.Button className='flex w-full items-center justify-between gap-2 rounded border border-primary/15 px-5 py-4 dark:border-white/15 lg:min-w-60'>
          <span className='inline-flex flex-col leading-tight'>
            <span>{languages.find((language) => language.key === activeLanguageKey)?.value}</span>
          </span>
