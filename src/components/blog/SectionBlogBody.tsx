...
import CommentPost from "@/components/blog/CommentPost"
      ...
      <div className='mx-auto max-w-4xl space-y-10'>
        ...
        <div className='space-y-5'>
          ...
          <ol className='ml-5 space-y-5 text-neutral-500'>
            {pathOr([], ["sectionFour", "points"], blogData).map((point, index) => (
              <li key={point}>
                {" "}
                <span>{index + 1}.</span>
                {point}
        ...
        <div className='relative overflow-hidden rounded-2xl p-14 shadow-xl'>
          ...
          <div className='text-2xl font-bold text-primary'>{pathOr("", ["quote"], blogData)}</div>
        </div>

        <div className='space-y-10'>
          {pathOr([], ["sectionFive"], blogData).map((section) => (
            <div key={pathOr("", ["title"], section)}>
              <Heading desc={pathOr("", ["description"], section)}>{pathOr("", ["title"], section)}</Heading>
            </div>
          ))}
        </div>
